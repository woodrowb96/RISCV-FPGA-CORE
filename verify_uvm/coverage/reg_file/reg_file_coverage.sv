...
*/
class reg_file_coverage extends uvm_object;
  `uvm_object_utils(reg_file_coverage);
  ...
  function new(string name = "reg_file_coverage");
    super.new(name);
    this.cg = new();
    this.reset_state();
  endfunction

  function void sample(reg_file_seq_item item);
    this.item = item;
    cg.sample();
  ...
  function void print_coverage_report();
    ...
    if (total < 100.0) begin
      ...
      $display("  read_during_write_reg_1:              %6.2f%%", cg.read_during_write_reg_1.get_coverage());
      $display("  read_during_write_reg_2:              %6.2f%%", cg.read_during_write_reg_2.get_coverage());
      $display("  next_cycle_read_after_write_reg_1:    %6.2f%%", cg.next_cycle_read_after_write_reg_1.get_coverage());
      $display("  next_cycle_read_after_write_reg_2:    %6.2f%%", cg.next_cycle_read_after_write_reg_2.get_coverage());
      $display("  x0_write_immunity:                    %6.2f%%", cg.x0_write_immunity.get_coverage());
      $display("  x0_rd_reg_1:                          %6.2f%%", cg.x0_rd_reg_1.get_coverage());
      $display("  x0_rd_reg_2:                          %6.2f%%", cg.x0_rd_reg_2.get_coverage());
      $display("========================================\n");
    end
    else begin
      $display("");
