...
if [[ ! -n "$DO_FILE" ]] ; then   #if a do file was not set by the -d flag
  if [[ -f "$PROJECT_ROOT_DIR/scripts/modelsim/$TEST_BENCH.tcl" ]] ; then  #if default do file exists
    DO_FILE="$PROJECT_ROOT_DIR/scripts/modelsim/$TEST_BENCH.tcl"   #get abs path to do file
  fi
fi
